...
class WPDA_API
{
    ...
    public function init()
    {
        ...
        register_rest_route( self::WPDA_NAMESPACE, 'table/datatable', array(
            'methods'             => array( \WP_REST_Server::READABLE, 'POST' ),
            'callback'            => array( $this, 'table_datatable' ),
            'permission_callback' => '__return_true',
            'args'                => array(
            'dbs'              => array(
            'required'          => true,
            ...
            'length'           => array(
            'required'          => false,
            'description'       => __( 'Number of records queried (0=all)', 'wp-data-access' ),
            'default'           => 10,
            'minimum'           => 0,
            'sanitize_callback' => function ( $param ) {
            return intval( sanitize_text_field( wp_unslash( $param ) ) );
    ...
    /**
     * Database table query for data tables.
     *
     ...
     * @return \WP_Error|\WP_REST_Response
     */
    public function table_datatable( $request )
    {
        // Get arguments (already sanitized and validated).
        $dbs = $request->get_param( 'dbs' );
        $tbl = $request->get_param( 'tbl' );
